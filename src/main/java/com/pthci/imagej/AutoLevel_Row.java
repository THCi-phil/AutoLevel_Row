...
public class AutoLevel_Row implements PlugInFilter {
	...
	public void process(byte[] pixels) {
		...
		//c.f. Invert_Image where we could just do for( int pixelPos=0; pixelPos<(width*height); pixelPos++ ) through the array.
		//here we actually want to x and y separately.
		//We can still speed up the scan by doing the y*width once for each row, rather than at every pixel
		//
		...
		for( int y=0; y<height; y++ ) {
			rowMin = 255 ; //reset each row
			rowMax = 0   ;
			//first pass along the row to find min and max in that row
			pixelPos = y*width;
			for( int x=0; x<width; x++ ) {
				testedPixelValue = pixels[pixelPos] & 0xff ;
				if( testedPixelValue < rowMin ) rowMin = testedPixelValue ;
				if( testedPixelValue > rowMax ) rowMax = testedPixelValue ;
				pixelPos++;
			} //end for x, min-max scan
			//then second pass to re-level the values
			gradient = (double)( (double)255.0 / ( (double)rowMax - (double)rowMin ) );
			pixelPos = y*width;
			for( int x=0; x<width; x++ ) {
				pixels[pixelPos] = (byte)( (int)Math.round( ( (pixels[pixelPos] & 0xff) -(double)rowMin )*gradient ) );
				pixelPos++;
			} //end for x, set re-level
		} //end for y
  ...
  //-----------------------------------------------------
	...
	public void process(short[] pixels) {
		...
		int rowMin = 65535 ; //set as the max possible, update with each value lower
		int rowMax =     0 ; //set as the min possible, update with each value larger
		int pixelPos = 0 ;
		int testedPixelValue ;
		double gradient ;
		for( int y=0; y<height; y++ ) {
			rowMin = 65535 ; //reset each row
			rowMax =     0 ;
			//first pass along the row to find min and max in that row
			pixelPos = y*width;
			for( int x=0; x<width; x++ ) {
			...
			//then second pass to re-level the values
			gradient = (double)( (double)65535.0 / ( (double)rowMax - (double)rowMin ) );
			pixelPos = y*width;
			for( int x=0; x<width; x++ ) {
				pixels[pixelPos] = (short)( (int)Math.round( ( (pixels[pixelPos] & 0xff) -(double)rowMin )*gradient ) );
				pixelPos++;
			} //end for x, set re-level
		} //end for y
  ...
  //-----------------------------------------------------
	...
	public void process( float[] pixels ) {
		...
		float rowMin = (float)1.0 ; //set as the max possible, update with each value lower
		float rowMax = (float)0.0 ; //set as the min possible, update with each value larger
		int   pixelPos = 0 ;
		float testedPixelValue ;
		float gradient ;
		for( int y=0; y<height; y++ ) {
			rowMin = (float)1.0 ; //reset each row
			rowMax = (float)0.0 ;
			//first pass along the row to find min and max in that row
			pixelPos = y*width;
			for( int x=0; x<width; x++ ) {
			...
			//then second pass to re-level the values
			gradient = (float)1.0 / ( rowMax - rowMin ) ;
			pixelPos = y*width;
			for( int x=0; x<width; x++ ) {
				pixels[pixelPos] = ( pixels[pixelPos] - rowMin )*gradient ;
				pixelPos++;
			} //end for x, set re-level
		} //end for y
...
/*=================================================================================*/
	...
	public void showAbout() {
		IJ.showMessage("AutoLevel Row",
			"Set each row scaled 0 to 255 (8bit example)"
		);
...
/*=================================================================================*/
	...
	public static void main(String[] args) throws Exception {
		...
		Class<?> clazz = AutoLevel_Row.class;
